...
#### 16. В чём разница между canvas и SVG?

**Ответ:**

**`Canvas` и `SVG`** — это два подхода для отображения графики в веб-браузерах, но они имеют принципиально разные механизмы рендеринга и применения.

Основные различия между `canvas` и `SVG`:

1. **Метод рендеринга**:
   - **`Canvas`** использует **растровую графику**. Это означает, что графика рисуется по пикселям в прямоугольной области (холсте). Каждое обновление холста требует перерисовки всей сцены. Это подходит для сложных графических операций, анимаций и игр.
   - **`SVG`** использует **векторную графику**. SVG представляет объекты в виде элементов DOM, и каждый элемент описывается векторными фигурами (линии, круги, многоугольники и т.д.). Векторные изображения сохраняют свою четкость при изменении масштаба и не требуют перерисовки сцены при изменении отдельного элемента.

2. **Поддержка интерактивности**:
   - **`SVG`** элементы — это часть DOM-дерева. Это означает, что к ним можно применять CSS-стили, JavaScript-события и анимации. Например, можно легко привязать события `click`, `hover` или другие взаимодействия с пользователем к любому элементу SVG.
   - **`Canvas`**, с другой стороны, не предоставляет нативных методов для работы с событиями на отдельных элементах, поскольку каждый элемент — это часть растрового изображения. Чтобы добавить интерактивность, нужно отслеживать координаты и состояние объектов вручную через JavaScript.

3. **Производительность**:
   - **`Canvas`** лучше подходит для сложной анимации и графики с большим количеством динамических элементов, например, игр или визуализаций в реальном времени. Это связано с тем, что `canvas` рендерит всю графику за один раз, не сохраняя информацию о каждом нарисованном элементе.
   - **`SVG`** может показывать ухудшение производительности при работе с большим количеством элементов. Поскольку каждый элемент является частью DOM, браузер должен управлять и рендерить каждый элемент отдельно, что может быть ресурсоемким, если на странице тысячи объектов.

4. **Масштабируемость и качество изображения**:
   - **`SVG`** — это векторная графика, поэтому она **масштабируется без потери качества**. SVG подходит для отображения на экранах с высоким разрешением, так как изображения будут оставаться четкими при любом увеличении.
   - **`Canvas`** — это растровая графика, которая состоит из пикселей. При масштабировании растровое изображение теряет качество (пикселизация), поскольку увеличивается размер пикселей.

5. **Использование**:
   - **`Canvas`** лучше всего подходит для задач, связанных с визуализацией данных в реальном времени, сложной анимацией, играми и графиками, где производительность имеет приоритет.
   - **`SVG`** чаще всего используется для построения простых иконок, диаграмм, инфографики и векторных изображений, которые требуют высокого качества и масштабируемости.
...
```
В этом примере создаётся синий прямоугольник на холсте, используя растровую графику.
...
```
Этот пример создаёт аналогичный синий прямоугольник, но с использованием векторной графики.

Когда использовать `Canvas` и когда `SVG`?
- Используйте **`canvas`**, когда вам нужно быстро рисовать множество графических элементов, обновлять их в реальном времени и не требуется масштабирование без потери качества. Это идеальный выбор для игр и анимаций.
- Используйте **`SVG`**, когда нужно обеспечить масштабируемость, высокое качество графики и вам требуется интерактивность на уровне отдельных элементов (например, диаграммы, иконки или векторные изображения).

Итог:
- **`Canvas`** — растровый метод рендеринга, подходит для динамических сцен, но требует перерисовки при каждом изменении.
- **`SVG`** — векторный метод рендеринга, идеально подходит для отображения масштабируемой графики и поддерживает интерактивность на уровне DOM-элементов.

##### Источники:

- [MDN по canvas](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API)
- [MDN по SVG](https://developer.mozilla.org/en-US/docs/Web/SVG).

---

#### 17. Что такое пустые элементы HTML?

**Пустые элементы** в HTML — это элементы, которые **не содержат контента** между открывающим и закрывающим тегами и обычно **не имеют закрывающего тега**. Такие элементы либо служат для вставки медиа или технической информации, либо выполняют вспомогательные функции, не требующие содержимого между тегами.
...
1. **`<img>`**:
   ...
   - Пример:
     ...
     ```
     Этот элемент не содержит вложенного контента, так как его основная функция — это отображение изображения, указанного в атрибуте `src`.

2. **`<br>`**:
   - Создаёт разрыв строки (перенос текста на новую строку).
   - Пример:
     ```html
     Текст на первой строке<br>Текст на второй строке.
     ```
     Элемент `<br>` не содержит контента, его задача — разрывать текст для создания новой строки.

3. **`<hr>`**:
   - Вставляет горизонтальную линию, которая визуально разделяет контент.
   - Пример:
     ```html
     <hr>
     ```
     Этот элемент тоже не содержит вложенного текста или других элементов.

4. **`<input>`**:
   - Используется для создания интерактивных элементов формы, таких как поля ввода, кнопки, чекбоксы и т.д.
   - Пример:
     ```html
     <input type="text" name="username">
     ```
     Элемент `<input>` не имеет содержимого, так как он представляет собой поле ввода данных.

5. **`<meta>`**:
   - Предоставляет метаданные о документе (например, кодировка, описание, ключевые слова).
   - Пример:
     ```html
     <meta charset="UTF-8">
     ```
     Этот элемент размещает информацию, которая не отображается на странице, и не требует содержимого.

Особенности пустых элементов:
- Пустые элементы **самозакрываются** или не имеют закрывающего тега. Например, `<img>` не требует записи `</img>`, а `<input>` не закрывается через `</input>`.
- Некоторые пустые элементы используются для структурирования (например, `<br>`, `<hr>`), а другие — для представления данных или предоставления информации (например, `<meta>`, `<img>`).
- В HTML5 разрешено не использовать слеш в самозакрывающихся тегах (например, `<img>`, а не `<img />`), хотя старые стандарты требовали этого.

Почему важны пустые элементы:
Пустые элементы играют важную роль в HTML, предоставляя полезные функции без необходимости содержимого между открывающим и закрывающим тегами. Они помогают управлять структурой страницы и её представлением, обеспечивая такие важные функции, как размещение изображений, разрыв строк, вставка метаданных и создание интерактивных элементов форм.

Итог:
**Пустые элементы** в HTML — это элементы, которые **не содержат контента** между тегами и часто не имеют закрывающего тега. К ним относятся такие элементы, как `<img>`, `<br>`, `<hr>`, `<input>` и `<meta>`, которые выполняют свои функции без необходимости иметь внутренний текст или другие элементы.

##### Источники:

- [MDN по HTML-элементам](https://developer.mozilla.org/en-US/docs/Web/HTML/Element).
